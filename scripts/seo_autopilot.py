...
- "meta_description": meta description (140-160 симв, с ключом и УТП)
...
- "toc": массив из 4-6 строк-заголовков разделов (для оглавления)
- "body_html": ОСНОВНОЙ текст в HTML. Используй <h2 id="s1">..</h2> (id=s1,s2,..),
  <h3>, <p>, <ul><li>, <strong>. 700-1100 слов. Обязательно: раздел про пользу/зачем,
  «как сделать пошагово через VideoAI» (загрузка фото на botisk.ru, ~60 секунд, первое бесплатно),
  «какое фото выбрать», и блок советов/сравнения. Внутри 1-2 ссылки <a href="/">VideoAI</a>
  или <a href="/#generator">создать видео</a>. id заголовков совпадают с порядком в "toc".
- "faq": массив из 3 объектов {{"q":"вопрос","a":"ответ 1-2 предложения"}}
